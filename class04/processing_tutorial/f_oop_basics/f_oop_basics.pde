...
void setup(){
  size(500,400);
  h = new Human("Angela", "female", 100., 200.);
  frameRate(60);
}
...
void mouseDragged(){
  ...
  println( "mouseDragged called - " + h.x + " - " + h.y );
}
void mousePressed(){
  h.move_myself(mouseX, mouseY);
  println( "mousePressed called - " + h.x + " - " + h.y );
}
void mouseReleased(){
  println("mouseReleased called");
}
